...
npm test                    # vitest run (100 test files, node env, src/**/*.test.ts)
...
Browser dashboard (src/app.tsx, React 19 + useAgent WebSocket)
   │
   ▼
export default { fetch, scheduled, queue }        src/server.ts:8599
   ...
   └── /*                       → ./public via ASSETS
   │
   ├── scheduled() every 10 min → crawl tick + idle tick + traffic-source fill
   │                              + analytics tick
   └── queue()                  → skill-fetch batch (src/skills/consumer.ts)
```

The idle tick skips itself while a generation is running; the traffic-source
fill deliberately does **not** — filling distribution channels during the
minutes the writer spends waiting on models is the point.
...
`generateArticle()` in `src/pipeline/writer.ts` (5.2k lines) is the spine; each
...
`src/pipeline/traffic-sources.ts` fills all 13 distribution channels per
published article against a KV ledger, independently of the pipeline steps that
first published it.

- **Machine channels** (`sitemap`, `indexnow`, `rss`, `websub`, `qa-json`) do
  real network/KV work and are **re-verified on every pass**, so a channel that
  silently failed at publish time gets refilled. `qa-json` rebuilds its payload
  from the live page's FAQPage JSON-LD when Step 22's write never landed.
- **Copy channels** (`pinterest`, `x`, `facebook`, `reddit`, `quora`,
  `youtube-short`, `newsletter`, `medium`) are generated by Kimi in two batched
  calls and stored as ready-to-paste artifacts. **Nothing auto-posts** — none of
  those platforms exposes a posting API to this project.

State lives in `traffic-sources:<kvKey>` (the ledger) and
`traffic-source:<id>:<kvKey>` (one channel's artifact). Each source retries
independently up to `MAX_ATTEMPTS` (5), so one bad channel can't strand the
others. Writes go through `mergeLedgers` (re-read + merge, not last-writer-wins)
because a fill can outrun its 60 s `traffic-sources:lock`. Entry points: a
fire-and-forget call at the top of `generateArticle()` (excluding the in-flight
kvKey) and the 10-minute cron. Every fill logs under activity-log role
`marketing`; the dashboard's "Traffic Sources" panel reads
...
- **KV `ARTICLES_KV`** — `<categorySlug>:<slug>` article HTML, `<kvKey>-b` variant B, `kimi-raw:<kvKey>` (48 h, raw model output for diagnosis), `editorial-report:<kvKey>`, `traffic-sources:<kvKey>` + `traffic-source:<id>:<kvKey>`, `escalation-dedup:*`, `sitemap:flat-sitemap`.
...
- **New `/api/*` routes need two edits.** Add the handler on the DO's `onRequest()` _and_ register the path in `proxyPaths`/the prefix check in the top-level `fetch` (`src/server.ts:8755`), or the Worker never forwards it. If it must work without a browser session, also add it to the `isMachineApi` exemption list.
...
`traffic-sources?limit=N`, `traffic-sources/fill` (`{ kvKey?, force? }`),
`traffic-source/<sourceId>/<kvKey>`.
