...
export default async function SerpPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  if (!historyId && keyword) {
    if (!creds) {
      ...
      try {
        ...
        error = 'Error while querying DataForSEO.';
      }
    }
  }

  // Auto-hydrate most recent saved search when page is loaded clean
  if (!historyId && !keyword && results.length === 0 && history.length > 0) {
    const latest = history[0];
    const saved = await getSerpResults<SerpItem>(latest.id);
    if (saved) {
      results = saved;
      activeEntry = latest;
      isFromHistory = true;
  ...
  const hasQuery = Boolean(historyId || keyword || isFromHistory || results.length > 0);
                ...
                {history.map((entry) => {
                  ...
                  return (
                    <a key={entry.id} href={`/dashboard/serp?history_id=${entry.id}`}
                      ...
                      <p className="text-[10px] text-slate-400 truncate mt-0.5">{entry.location} · {entry.count} res.</p>
