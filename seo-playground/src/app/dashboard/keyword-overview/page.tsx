...
export default async function KeywordOverviewPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  if (!historyId && keywords.length === 0 && items.length === 0) {
    if (history.length > 0) {
      const latest = history[0];
      const saved = await getKwOverviewResults<KwOverviewItem>(latest.id);
      if (saved) {
        items = saved;
        activeEntry = latest;
        isFromHistory = true;
  ...
  const hasQuery = Boolean(historyId || keywords.length > 0 || isFromHistory || items.length > 0);
