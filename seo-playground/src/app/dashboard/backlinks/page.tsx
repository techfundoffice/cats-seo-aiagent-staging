...
export default async function BacklinksPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  if (!historyId && !target && !summary && links.length === 0) {
    if (history.length > 0) {
      const latest = history[0];
      const savedSummary = await getBacklinksResult<BacklinksSummary>(latest.id);
      const savedLinks = await getBacklinksLinks<BacklinkItem>(latest.id);
      if (savedSummary) {
        summary = savedSummary;
        links = savedLinks ?? [];
        activeEntry = latest;
        linksTotal = latest.linksTotal ?? links.length;
        isFromHistory = true;
      }
    }
  }

  const hasQuery = Boolean(historyId || target || isFromHistory || summary || links.length > 0);
    ...
    <div className="space-y-6">
      ...
      <SearchForm className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm space-y-4" btnLabel="Analyze" btnClassName="w-full bg-slate-900 text-white font-black uppercase tracking-widest text-xs py-3 rounded-xl hover:bg-blue-600 transition-colors">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-xs font-black uppercase tracking-widest text-slate-400 mb-1.5">Link count</label>
          ...
          {summary && (
            <>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                <StatCard label="Backlinks brokens" value={summary.broken_backlinks} />
                <StatCard label="Pages brokenes" value={summary.broken_pages} />
                <StatCard label="Subnets" value={summary.referring_subnets} />
              </div>
              ...
              {(linkTypeEntries.length > 0 || tldEntries.length > 0) && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  {tldEntries.length > 0 && (
                    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
                      <h2 className="text-xs font-black uppercase tracking-widest text-slate-400 mb-4">Referring domain TLDs</h2>
                      <div className="space-y-2">
