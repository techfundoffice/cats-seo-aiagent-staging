...
export default async function CompetitorsPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  if (!historyId && !target && items.length === 0 && history.length > 0) {
    const latest = history[0];
    const saved = await getCompetitorsResults<CompetitorItem>(latest.id);
    if (saved) {
      items = saved;
      activeEntry = latest;
      isFromHistory = true;
  ...
  const hasQuery = Boolean(historyId || target || isFromHistory || items.length > 0);
