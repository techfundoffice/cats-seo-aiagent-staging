...
export default async function KeywordDifficultyPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const hasQuery = Boolean(historyId || keywords || isFromHistory || items.length > 0);
  ...
  if (!historyId && !keywords && items.length === 0) {
    if (history.length > 0) {
      const latest = history[0];
      const saved = await getKwDifficultyResults<DifficultyItem>(latest.id);
      if (saved) {
        items = saved;
        activeEntry = latest;
        isFromHistory = true;
