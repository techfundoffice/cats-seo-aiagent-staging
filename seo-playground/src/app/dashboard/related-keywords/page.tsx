...
export default async function RelatedKeywordsPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  if (historyId) {
    const saved = await getRelatedKwResults<RelatedKeywordItem>(historyId);
    if (saved) {
      items = saved;
      isFromHistory = true;
      const history = await getRelatedKwHistory();
  ...
  const hasQuery = Boolean(historyId || keyword || isFromHistory || items.length > 0);
  ...
  if (!historyId && keyword) {
    if (!creds) {
      ...
      if (!error && items.length > 0) {
        ...
        await saveRelatedKwSearch(entry, items);
      }
    }
  }
  ...
  if (!historyId && !keyword && items.length === 0 && history.length > 0) {
    const latest = history[0];
    const saved = await getRelatedKwResults<RelatedKeywordItem>(latest.id);
    if (saved) {
      items = saved;
      activeEntry = latest;
      isFromHistory = true;
