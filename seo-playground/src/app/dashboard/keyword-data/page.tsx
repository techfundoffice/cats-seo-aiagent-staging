...
export default async function KeywordDataPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const hasParamsQuery = historyId || (seType === 'keywords_for_site' ? params.target?.trim() : params.keywords?.trim());
  const hasQuery = Boolean(hasParamsQuery || isFromHistory || items.length > 0);
  ...
  if (!historyId && !hasQuery && items.length === 0 && historyIndex.length > 0) {
    const latest = historyIndex[0];
    const saved = await getKdResults<KeywordItem>(latest.id);
    if (saved) {
      items = saved;
      activeEntry = latest;
      isFromHistory = true;
  ...
  const formDefaults = {
    ...
    targetType: sourceParams.target_type ?? 'site', location: sourceParams.location ?? 'United States',
    language: sourceParams.language ?? 'English', searchPartners: sourceParams.search_partners ?? 'false',
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-100 bg-slate-50">
                    ...
                    <th className="px-4 py-3 text-center text-[10px] font-black uppercase tracking-widest text-slate-400">Compet.</th>
